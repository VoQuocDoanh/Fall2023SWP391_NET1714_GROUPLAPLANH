...
function PaymentSuccessOrder() {
    ...
    useEffect(() => {
        const handleActivationDemo = async () => {
            await axios.post("http://localhost:8080/api/v1/paypal/user/order/success", { paymentId: paymentId, payerID: PayerID, id: id, flag: "demo" })
                .then((res) => {
        ...
        const handleActivationFull = async () => {
            await axios.post("http://localhost:8080/api/v1/paypal/user/order/success", { paymentId: paymentId, payerID: PayerID, id: id, flag: "full" })
                .then((res) => {
                    console.log(res.data)
                })
                .catch((error) => {
        ...
        if(method == "demo"){
            handleActivationDemo()
        }else{
            handleActivationFull()
        }
    }, [])
