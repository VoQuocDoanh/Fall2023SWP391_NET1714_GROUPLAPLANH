        ...
        <div className={cx("list-header")}>
            ...
            {list ?
                <div className={cx("listbeat")}>
                    {list.map((item) => {
                        return <ListBeatBox id={item.id} name={item.beatName} genre={item.genre} price={item.price} view={(item.view / 2).toFixed()} like={item.totalLike} play={item.play} setPlay={setPlay} onClick={() => handleClickAudio(item)} handleLike={() => handleLike(item.id)} rating={item.rating} vocalRange={item.vocalRange} fullName={item.user.fullName} />
                    })}
